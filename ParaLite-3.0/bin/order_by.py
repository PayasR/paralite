        ...
        try:
            ...
            if flag == 1:
                ...
                while i >= 0:
                    ...
                    if pos[1] == conf.INT:
                        whole_data.sort(
                            key = lambda l: string.atoi(
                                l.split(self.db_col_sep)[pos[0]]),
                            reverse = pos[2])
                    elif pos[1] == conf.FLOAT or pos[1] == conf.REAL:
                        whole_data.sort(
                            key = lambda l: float(l.split(self.db_col_sep)[pos[0]]),
                            reverse = pos[2])
                    else:
                        whole_data.sort(
                            key = lambda l: l.split(self.db_col_sep)[pos[0]],
                            reverse = pos[2])
                    i -= 1
            else:
                sort_key = []
                ...
                whole_data.sort(key = lambda l: eval(",".join(sort_key)),
                                reverse = key_pos[0][2])
    ...
    def distribute_data(self):
        ...
        if self.dest == conf.DATA_TO_ONE_CLIENT:
            ...
            dload_client.dload_client().load_internal(
                ...
                row_sep, col_sep, False, "0", LOG_DIR)
...
def main():
    ...
    ParaLiteLog.init("%s/orderby-%s-%s.log" % (proc.log_dir, gethostname(), cur_time),
                     logging.DEBUG)
